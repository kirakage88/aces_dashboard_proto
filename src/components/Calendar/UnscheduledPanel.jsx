...
export default function UnscheduledPanel({ unscheduledProjects, onProjectClick }) {
  return (
    <div className="lg:col-span-4 space-y-6 md:space-y-8">
      <div className="bg-white dark:bg-dark-card border border-slate-100 dark:border-dark-border rounded-[2rem] md:rounded-[3rem] shadow-sm p-5 md:p-8 flex flex-col max-h-[400px] md:max-h-[640px] transition-colors">
        <div className="flex items-center justify-between mb-4 md:mb-6">
          <h3 className="font-black text-slate-900 dark:text-dark-text tracking-tight flex items-center gap-2 text-sm md:text-base transition-colors">
        ...
        <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar space-y-2 md:space-y-3">
          {unscheduledProjects.length === 0 ? (
            <div className="py-8 md:py-12 text-center text-slate-300 dark:text-dark-muted transition-colors">
            ...
            unscheduledProjects.map((proj) => (
              ...
              >
                <div className="flex justify-between items-start gap-2 mb-1 md:mb-2">
                  <h4 className="font-black text-slate-800 dark:text-dark-text text-[10px] md:text-xs leading-snug group-hover:text-[#550000] dark:group-hover:text-dark-accent transition-colors truncate">
                    {proj.name}
                  </h4>
                  <StatusBadge status={proj.status} />
                </div>
                <div className="flex justify-between items-center text-[9px] md:text-[10px] text-slate-400 dark:text-dark-muted font-black transition-colors">
